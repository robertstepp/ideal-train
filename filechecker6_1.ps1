<#
    ...
    The manifest groups its file listing into tiers, so that everything cyber
    counts as executable sorts to the top and anomalies that need justifying
    before the media crosses are broken out on their own. Per that ruling,
    "executable" means Windows executables and scripts: libraries and drivers
    are not, nothing Linux is, and bytecode is not evaluated.

    Tiers are a manifest concern only. Hashing, the worker protocol, the
    .hashes.csv and the .hashes.csv.sha512 sidecar are unchanged from v6.0 --
    classification runs in the parent once the CSV and sidecar are written and
    the workers have exited.

    See README.md for the workflow, the output files, and the change history.

    Usage:
        powershell -File filechecker6_1.ps1
        powershell -File filechecker6_1.ps1 -DebugMode
        powershell -File filechecker6_1.ps1 -BasePath "D:\Transfer"
        powershell -File filechecker6_1.ps1 -DebugMode -SkipArchiveContents
        powershell -File filechecker6_1.ps1 -ReportExtraFiles
        powershell -File filechecker6_1.ps1 -IncludeSystemFiles
        powershell -File filechecker6_1.ps1 -Parallel 8
        powershell -File filechecker6_1.ps1 -Parallel 1#>
...
#
#  MANIFEST ONLY: runs after the CSV and sidecar are written and the
#  workers have exited, so it cannot affect either file or the hashing.
#
#  Scope, per the cyber ruling: "executable" means Windows executables
#  and scripts. Libraries and drivers fall to Data; Linux packages sit
#  with the containers; bytecode is not evaluated; ELF and Mach-O are
#  not detected, having no tier to land in. Signatures are read for one
#  job only -- catching a Windows executable wearing another extension.
# =====================================================================

# An array, not [ordered]@{1='x'}: $dict[1] on an ordered hashtable resolves as a
# POSITIONAL INDEX, not a key, which silently shifts every tier label by one.
$script:Tiers = @(
    [pscustomobject]@{ Number = 1; Name = 'Windows executables' }
    [pscustomobject]@{ Number = 2; Name = 'Scripts' }
    [pscustomobject]@{ Number = 3; Name = 'Containers' }
    [pscustomobject]@{ Number = 4; Name = 'ANOMALIES - justify before transfer' }
    [pscustomobject]@{ Number = 5; Name = 'Data' }
)

# Multi-part extensions first, so '.tar.gz' wins over '.gz'.
#
# Reading a manifest, note what the ruling excludes: .dll/.sys/.ocx/.cpl/.drv are
# not called out (nothing launches them directly), .rpm/.deb sit in Containers
# despite their root-level scriptlets, and Java is absent entirely.
$script:TierExtensions = @(
    @{ Tier = 3; Ext = @('.tar.gz', '.tar.bz2', '.tar.xz') }
    ...
    @{ Tier = 3; Ext = @('.cab', '.psf', '.tgz', '.tar', '.cvd', '.cld', '.jdb',
                         '.zip', '.iso', '.wim', '.esd', '.appx', '.msix', '.7z',
                         '.gz', '.rpm', '.deb', '.nbin') }
)
...
# Where a PE header is expected, so finding one is not a mismatch. Anything
# OUTSIDE this list that proves to be a PE is an anomaly -- a .dat or .cab
# holding one is the case worth catching. Note .dll and .sys are here despite
# living in Data, or every library would be falsely flagged.
$script:PeExpectedExtensions = @('.exe', '.com', '.scr', '.pif', '.dll', '.sys',
                                 '.ocx', '.cpl', '.drv', '.efi')

function Get-FileSignature {
    # Names the format from the first bytes. Eight is enough for both signatures,
    # so this is a trivial read next to hashing the file.
    param([Parameter(Mandatory)][string] $Path)
    try {
        ...
        # Two signatures, not eleven: the ruling removed every tier the others
        # fed, so reading them would imply judgements the taxonomy no longer makes.
...
function Get-EntryClassification {
    # Returns @{ Tier; Reason }; Reason is set only for the anomaly tier.
    #
    # $FullPath is supplied only for real files on disk. Archive-internal entries
    # have none, so they are classified on extension alone -- stated in the
    # manifest, so a clean anomaly tier is not misread as proof the archives are.
    param(
        [Parameter(Mandatory)][string] $RelativePath,
        [string] $FullPath
    )
    ...
    # The high-value check: a Windows executable wearing another extension.
    # Tested against PeExpectedExtensions rather than the tier, so a .dll in Data
    # is not reported -- a PE is exactly what a .dll should be.
    if ($sig -eq 'PE' -and $ext -and ($script:PeExpectedExtensions -notcontains $ext)) {
        [void]$reasons.Add("PE header in a '$ext' file - extension does not match content")
    }
    # update.txt.exe
    if ($extTier -in @(1, 2)) {
    ...
    if ($reasons.Count -gt 0) {
        return [pscustomobject]@{ Tier = 4; Reason = ($reasons -join '; ') }
    }
...
function Set-InitialFileAutomatic {
    ...
    # Classify, then group. Nothing above this point changes: the CSV is written,
    # the sidecar computed, and every hashing worker has exited.
    ...
    [void]$classified.Add([pscustomobject]@{ Path = $csvName;     Tier = 5; Reason = '' })
    [void]$classified.Add([pscustomobject]@{ Path = $sidecarName; Tier = 5; Reason = '' })

    $anomalyCount  = @($classified | Where-Object { $_.Tier -eq 4 }).Count
    ...
    foreach ($tier in $script:Tiers) {
        ...
        $flag = if ($tier.Number -eq 4 -and $n -gt 0) { '   <-- REVIEW' } else { '' }
    ...
    if ($anomalyCount -gt 0) {
        ...
        $popupMsg += "`r`n`r`nANOMALIES: $anomalyCount file(s) need justifying before " +
                     "transfer. See Tier 04 in the manifest."
        $icon = 'Warning'
    }
    if ($script:ArchiveWarnings.Count -gt 0) {
