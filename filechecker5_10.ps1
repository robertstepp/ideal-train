<#
    ...
    File Checker v5.9  (CSV integrity sidecar)

    Changes over 5.8:
      * At the end of an automatic initial build, a sidecar file is
        written alongside the CSV:
            20260514_2226-initial.hashes.csv.sha512
        It contains SHA-512 hashes of two files in sha512sum format:
            <hash>  <csv-filename>
            <hash>  <ps1-filename>
        Burn this file to disc with the CSV. The manifest is excluded
        (it is informational and not burned per the stated workflow).
      * At the start of every verification run (Basic and Comprehensive),
        the script locates the sidecar, rehashes the CSV and the PS1,
        and compares against the recorded values.
      * Missing sidecar: blocked entirely (workflow error; sidecar was
        not burned to disc).
      * Script hash mismatch: hard block with a clear message that the
        script may contain malicious code.
      * CSV hash mismatch: warning dialog explains the CSV may have
        been modified; Yes/No to continue. Default button is No.
      * *.sha512 added to ScriptOutputPatterns so sidecar files are
        excluded from scan listings.

    Changes over 5.7:
      * Invoke-FileScan now skips files matching the script's own output
        patterns before hashing them:
            *-initial.hashes.csv
            *-initial.manifest.log
            *-rescan.hashes.csv
            *-fileverification.log
            debug.log
        These files live next to the script and will always appear as New
        on a Comprehensive rescan when the script folder is the scan target.
        The exclusion list is defined in $script:ScriptOutputPatterns and
        applies to both the initial build and both Comprehensive scan passes.

    Carried over from 5.7:
      * Basic/Comprehensive mode naming.
      * Both CSVs always kept in Comprehensive mode.
      * Fixed: rescanLookup built with .ToArray() on already-unwrapped
        object[] returned $null; changed to iterate $rescanList directly.
...
function New-SidecarFile {
    ...
    if (-not $scriptPath) {
        $scriptPath = Join-Path (Get-ParentScriptFolder) `
...
function Test-SidecarFile {
    ...
    foreach ($entry in $expected.GetEnumerator()) {
        $name     = $entry.Key
        $expHash  = $entry.Value
        $fullPath = Join-Path (Get-ParentScriptFolder) $name
...
function Set-InitialFileAutomatic {
    ...
    $sidecarPath = New-SidecarFile -CsvPath $csvPath
    Write-Debug "Sidecar: $sidecarPath"
    ...
    $popupMsg = "Initial listing built.`r`nEntries: $($output.Count)`r`nCSV: $csvPath`r`nSidecar: $sidecarPath`r`n`r`nBurn both the CSV and the .sha512 file to disc."
    if ($script:ArchiveWarnings.Count -gt 0) {
...
function Compare-HashEntryThorough {
    ...
    $rescanLookup  = @{}
    foreach ($p in $rescanList)           { $rescanLookup[$p.FilePath]  = $p.Hash }
