<#
    ...
    File Checker v6.1 (PROPOSED V2 - not for production use)

    Based on v6.0. PROPOSAL under review by cyber. The ONLY change from v6.0 is
    the readable manifest: its file listing is now grouped into tiers so that
    everything capable of executing on the receiving side sorts to the top, with
    a separate tier for anomalies that need justifying before the media crosses.

    Nothing else is affected. Hashing, the parallel worker protocol, the
    .hashes.csv, and the .hashes.csv.sha512 integrity sidecar are unchanged from
    v6.0 -- classification runs in the parent after the CSV and sidecar have
    been written and every worker has exited.

    Scope is Windows and Linux. No macOS file type appears in any tier; Mach-O
    is detected solely to raise an anomaly.

    (Supersedes the first proposal, which was mistakenly based on 5.13 and is
    kept at Archive\filechecker6_1_proposed.ps1.)
    ...
    Usage:
        powershell -File filechecker6_1_proposedV2.ps1
        powershell -File filechecker6_1_proposedV2.ps1 -DebugMode
        powershell -File filechecker6_1_proposedV2.ps1 -BasePath "D:\Transfer"
        powershell -File filechecker6_1_proposedV2.ps1 -DebugMode -SkipArchiveContents
        powershell -File filechecker6_1_proposedV2.ps1 -ReportExtraFiles
        powershell -File filechecker6_1_proposedV2.ps1 -IncludeSystemFiles
        powershell -File filechecker6_1_proposedV2.ps1 -Parallel 8
        powershell -File filechecker6_1_proposedV2.ps1 -Parallel 1
#>
...
# =====================================================================
#  Manifest classification  (v6.1)
#
#  MANIFEST ONLY. This changes how the readable listing is grouped and
#  ordered. It does not touch what gets hashed, the CSV, or the .sha512
#  sidecar -- classification runs at manifest-build time, after the CSV
#  has already been written, so it cannot affect either file. It is also
#  entirely outside the parallel hashing path: workers return the same
#  {FilePath; Hash} records they always did, and classification happens
#  once in the parent after every worker has finished.
#
#  Scope: Windows and Linux. No macOS file type appears in any tier;
#  Mach-O is detected purely to raise an anomaly, because an Apple
#  binary on this media has no legitimate explanation.
# =====================================================================

# An array, not a hashtable keyed on integers. [ordered]@{1='x'} looks right but
# $dict[1] resolves as a POSITIONAL INDEX rather than a key lookup, which silently
# shifts every tier label by one -- the entries group correctly and the headings
# above them are all wrong. Objects make the lookup unambiguous.
$script:Tiers = @(
    [pscustomobject]@{ Number = 1; Name = 'Binaries & installers' }
    [pscustomobject]@{ Number = 2; Name = 'Scripts' }
    [pscustomobject]@{ Number = 3; Name = 'Libraries & drivers' }
    [pscustomobject]@{ Number = 4; Name = 'Containers' }
    [pscustomobject]@{ Number = 5; Name = 'ANOMALIES - justify before transfer' }
    [pscustomobject]@{ Number = 6; Name = 'Data' }
)

# Multi-part extensions are listed first so '.tar.gz' wins over '.gz'.
$script:TierExtensions = @(
    @{ Tier = 4; Ext = @('.tar.gz', '.tar.bz2', '.tar.xz') }
    @{ Tier = 1; Ext = @('.exe', '.com', '.scr', '.pif', '.msi', '.msp', '.msu',
                         '.rpm', '.deb', '.nbin') }
    @{ Tier = 2; Ext = @('.nasl', '.inc', '.ps1', '.psm1', '.bat', '.cmd', '.vbs',
                         '.vbe', '.wsf', '.wsh', '.js', '.jse', '.hta', '.sh',
                         '.bash', '.ksh', '.zsh', '.py', '.pl', '.rb') }
    @{ Tier = 3; Ext = @('.dll', '.sys', '.ocx', '.cpl', '.drv', '.efi', '.so',
                         '.ko', '.jar', '.class') }
    @{ Tier = 4; Ext = @('.cab', '.psf', '.tgz', '.tar', '.cvd', '.cld', '.jdb',
                         '.zip', '.iso', '.wim', '.esd', '.appx', '.msix', '.7z', '.gz') }
)

# Extensions that, when they appear immediately before an executable one,
# indicate a filename dressed up to look harmless (update.txt.exe).
$script:DecoyExtensions = @('.txt', '.pdf', '.doc', '.docx', '.xls', '.xlsx',
                            '.jpg', '.jpeg', '.png', '.gif', '.xml', '.log', '.csv')

function Get-FileSignature {
    # Reads the first few bytes and names the format. Eight bytes is enough for
    # every signature below, so this is a trivial read next to hashing the file.
    param([Parameter(Mandatory)][string] $Path)
    try {
        ...
        if ($hex.StartsWith('4D5A'))           { return 'PE'     }   # MZ
        if ($hex.StartsWith('7F454C46'))       { return 'ELF'    }
        if ($hex.StartsWith('FEEDFACE') -or $hex.StartsWith('FEEDFACF') -or
            $hex.StartsWith('CEFAEDFE') -or $hex.StartsWith('CFFAEDFE')) { return 'Mach-O' }
        if ($hex.StartsWith('CAFEBABE'))       { return 'Java-or-Mach-O' }
        if ($hex.StartsWith('2321'))           { return 'shebang' }  # #!
        if ($hex.StartsWith('EDABEEDB'))       { return 'RPM'    }
        if ($hex.StartsWith('213C617263683E')) { return 'ar'     }   # !<arch>, .deb
        if ($hex.StartsWith('4D534346'))       { return 'CAB'    }   # MSCF
        if ($hex.StartsWith('D0CF11E0'))       { return 'OLE'    }   # .msi/.msp
        if ($hex.StartsWith('504B0304'))       { return 'ZIP'    }
        if ($hex.StartsWith('436C616D4156'))   { return 'ClamAV-VDB' }
        if ($hex.StartsWith('1F8B'))           { return 'gzip'   }
        return 'data'
    } catch {
        Write-Debug "Could not read signature of '$Path': $_"
        return $null
    }
}

function Get-EntryClassification {
    # Returns @{ Tier; Reason }. Reason is populated only for Tier 5.
    #
    # $FullPath is supplied only for real files on disk. Archive-internal
    # entries have no path to read, so they are classified on extension alone
    # -- noted in the manifest so nobody reads a clean Tier 5 as proof that
    # nothing is hiding inside the archives.
    param(
        [Parameter(Mandatory)][string] $RelativePath,
        [string] $FullPath
    )
    ...
    $extTier = 6
    $matchedExt = $null
    foreach ($group in $script:TierExtensions) {
        foreach ($candidate in $group.Ext) {
            if ($lower.EndsWith($candidate)) { $extTier = $group.Tier; $matchedExt = $candidate; break }
        }
        if ($matchedExt) { break }
    }

    # --- signature, real files only ---------------------------------------
    $sig = $null
    if ($FullPath -and (Test-Path -LiteralPath $FullPath -PathType Leaf)) {
        $sig = Get-FileSignature -Path $FullPath
    }

    # --- anomaly rules -----------------------------------------------------
    if ($name.IndexOf([char]0x202E) -ge 0) {
        [void]$reasons.Add('right-to-left override character in filename')
    }
    if (-not $ext) {
        $note = if ($sig -eq 'shebang') { 'no extension (shebang script)' } else { 'no extension' }
        [void]$reasons.Add($note)
    }
    if ($sig -eq 'Mach-O') {
        [void]$reasons.Add('Mach-O binary - macOS is out of scope for this transfer')
    }
    if ($sig -eq 'ELF' -and -not (Test-IsArchiveInternalEntry $RelativePath)) {
        [void]$reasons.Add('loose ELF binary, outside any package')
    }
    # The high-value check: extension says data, content says code.
    if ($extTier -eq 6 -and $ext -and $sig -in @('PE', 'ELF', 'Mach-O')) {
        [void]$reasons.Add("$sig header in a '$ext' file - extension does not match content")
    }
    # update.txt.exe
    if ($extTier -in @(1, 2, 3)) {
        $stem = [System.IO.Path]::GetFileNameWithoutExtension($lower)
        $prev = [System.IO.Path]::GetExtension($stem)
        if ($prev -and ($script:DecoyExtensions -contains $prev)) {
            [void]$reasons.Add("double extension '$prev$ext'")
        }
    }

    if ($reasons.Count -gt 0) {
        return [pscustomobject]@{ Tier = 5; Reason = ($reasons -join '; ') }
    }
    return [pscustomobject]@{ Tier = $extTier; Reason = '' }
}
...
function Set-InitialFileAutomatic {
    ...
    # First line of the manifest. v6.1 drops the "Data Transfer Name" prompt
    # that used to ask for this -- one fewer dialog between picking a folder and
    # the scan starting. The scan root and build timestamp on the lines below
    # already identify the run.
    $transferName = "Initial hash listing"
    ...
    # v6.1: classify every entry, then group. Nothing above this point changes
    # -- the CSV has already been written and the sidecar already computed, and
    # every hashing worker has already exited.
    $classified = New-Object System.Collections.Generic.List[object]
    foreach ($e in $output) {
        # Archive-internal entries have no file on disk to read a signature from.
        $full = if (Test-IsArchiveInternalEntry $e.FilePath) { $null }
                else { Join-Path $scanRoot $e.FilePath }
        $c = Get-EntryClassification -RelativePath $e.FilePath -FullPath $full
        [void]$classified.Add([pscustomobject]@{
            Path = $e.FilePath; Tier = $c.Tier; Reason = $c.Reason
        })
    }
    # Written after the scan, so they are never scan results. Both are data.
    [void]$classified.Add([pscustomobject]@{ Path = $csvName;     Tier = 6; Reason = '' })
    [void]$classified.Add([pscustomobject]@{ Path = $sidecarName; Tier = 6; Reason = '' })

    $anomalyCount  = @($classified | Where-Object { $_.Tier -eq 5 }).Count
    $internalCount = @($classified | Where-Object { Test-IsArchiveInternalEntry $_.Path }).Count

    [void]$lines.Add(("=" * 21))
    [void]$lines.Add("Summary by tier:")
    foreach ($tier in $script:Tiers) {
        $n = @($classified | Where-Object { $_.Tier -eq $tier.Number }).Count
        $flag = if ($tier.Number -eq 5 -and $n -gt 0) { '   <-- REVIEW' } else { '' }
        [void]$lines.Add(("  Tier {0:00}  {1,-38}{2,8}{3}" -f $tier.Number, $tier.Name, $n, $flag))
    }
    [void]$lines.Add("")
    if ($internalCount -gt 0) {
        [void]$lines.Add("  Note: $internalCount entr$(if($internalCount -eq 1){'y is'}else{'ies are'}) inside an archive and")
        [void]$lines.Add("        classified on extension alone - no signature check is possible")
        [void]$lines.Add("        without a file on disk to read.")
        [void]$lines.Add("")
    }

    foreach ($tier in $script:Tiers) {
        $items = @($classified | Where-Object { $_.Tier -eq $tier.Number } | Sort-Object Path)
        if ($items.Count -eq 0) { continue }
        [void]$lines.Add(("=" * 21))
        [void]$lines.Add(("Tier {0:00} - {1}  ({2})" -f $tier.Number, $tier.Name, $items.Count))
        foreach ($i in $items) {
            [void]$lines.Add("  $($i.Path)")
            if ($i.Reason) { [void]$lines.Add("      ^ $($i.Reason)") }
        }
        [void]$lines.Add("")
    }
    # Trailing blank is re-added below where needed; drop this one so the
    # archive-warnings block does not end up double-spaced.
    ...
    if ($anomalyCount -gt 0) {
        # Surface this on screen, not just in the manifest. The whole point of
        # Tier 5 is that it should not be discoverable only by reading to the
        # bottom of a sixty-thousand-line listing.
        $popupMsg += "`r`n`r`nANOMALIES: $anomalyCount file(s) need justifying before " +
                     "transfer. See Tier 05 in the manifest."
        $icon = 'Warning'
    }
    if ($script:ArchiveWarnings.Count -gt 0) {
