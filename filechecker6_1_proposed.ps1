<#
    ...
    File Checker v6.1 (PROPOSED - not for production use)
    ...
    PROPOSAL under review by cyber. The ONLY change from v5.13 is the readable
    manifest: its file listing is now grouped into tiers so that everything
    capable of executing on the receiving side sorts to the top, with a separate
    tier for anomalies that need justifying before the media crosses.

    Nothing else is affected. The hashing, the .hashes.csv, and the
    .hashes.csv.sha512 integrity sidecar are byte-for-byte identical to v5.13 --
    classification runs after the CSV and sidecar have already been written.

    Scope is Windows and Linux. No macOS file type appears in any tier; Mach-O
    is detected solely to raise an anomaly.

    See README.md for the workflow, and the taxonomy document for the tier
    definitions and the open questions behind them.

    Usage:
        powershell -File filechecker6_1_proposed.ps1
        powershell -File filechecker6_1_proposed.ps1 -DebugMode
        powershell -File filechecker6_1_proposed.ps1 -BasePath "D:\Transfer"
        powershell -File filechecker6_1_proposed.ps1 -SkipArchiveContents
        powershell -File filechecker6_1_proposed.ps1 -ReportExtraFiles#>
...
#  has already been written, so it cannot affect either file.
#
...
function Set-InitialFileAutomatic {
    ...
    # -- the CSV has already been written and the sidecar already computed.
    ...
    # Trailing blank is added again below; drop this one so spacing stays even.
